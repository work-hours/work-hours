...
export default function TimeLogOffCanvas({ open, mode, onClose, projects, tasks, timeLog }: TimeLogOffCanvasProps) {
  ...
  const handleStartTimeChange = (date: Date | null) => {
    if (date) {
      const currentStart = new Date(data.start_timestamp)
      const localDate = new Date(currentStart.getFullYear(), currentStart.getMonth(), currentStart.getDate(), date.getHours(), date.getMinutes())
      setData('start_timestamp', localDate.toISOString())
      if (data.end_timestamp) {
        const currentEnd = new Date(data.end_timestamp)
    ...
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent side="right" className="overflow-y-auto bg-white pr-6 pb-8 pl-6 sm:max-w-md md:max-w-lg dark:bg-neutral-900">
        ...
        <form className="mt-4 flex flex-col gap-6" onSubmit={submit}>
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              ...
              <SearchableSelect
                ...
                onChange={(value) => { setData('project_id', parseInt(value)); setData('task_id', null) }}
                options={projects}
            ...
            <div className="grid gap-2">
              <Label htmlFor="task_id" className="text-sm font-medium">Task (Optional)</Label>
              <SearchableSelect
                id="task_id"
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="grid gap-2">
              ...
              <DatePicker
                ...
                required
                disabled={processing}
                customInput={<CustomInput id="log_date" icon={<Clock className="h-4 w-4 text-muted-foreground" />} required disabled={processing} />}
              />
            ...
            <div className="grid gap-2">
              ...
              <DatePicker
                ...
                required
                disabled={processing}
                customInput={<CustomInput id="start_time" icon={<Clock className="h-4 w-4 text-muted-foreground" />} required disabled={processing} />}
              />
            </div>
            <div className="grid gap-2">
              ...
              <DatePicker
                ...
                customInput={<CustomInput id="end_time" icon={<Timer className="h-4 w-4 text-muted-foreground" />} disabled={processing} />}
              />
          ...
          <div className="grid gap-2">
            <Label htmlFor="note" className="text-sm font-medium">Note {data.task_id ? '(Optional)' : ''}</Label>
            <Input id="note" value={data.note} onChange={(e) => setData('note', e.target.value)} required={!data.task_id} disabled={processing} />
          ...
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="non_billable" className="text-sm font-medium">Mark as non-billable</Label>
          </div>
          ...
          <div className="mt-2 flex justify-end gap-3">
            <button type="button" className="inline-flex h-9 items-center rounded-md border px-3 text-sm" onClick={onClose} disabled={processing}>Cancel</button>
            <SubmitButton
              loading={processing}
