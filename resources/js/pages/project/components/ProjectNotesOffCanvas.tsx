...
export default function ProjectNotesOffCanvas({ projectId, open, onClose }: Props) {
    ...
    return (
        <div
            className={`fixed inset-y-0 right-0 z-50 w-full transform bg-white shadow-xl transition-transform duration-300 sm:w-[420px] ${open ? 'translate-x-0' : 'translate-x-full'}`}
        >
            <div className="flex h-full flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    ...
                    <div className="space-y-3">
                        {notes.length === 0 && !loading && <div className="text-center text-sm text-gray-500">No notes yet.</div>}
                        {notes.map((note) => (
                            <div key={note.id} className="rounded-md border p-3 hover:shadow-sm">
                                ...
                                {editingId === note.id ? (
                                    ...
                                    <div className="text-sm whitespace-pre-wrap text-gray-800">{note.body}</div>
                                )}
                            </div>
                        ))}
                    </div>
