import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle } from '@/components/ui/sheet'
import ProjectTimeLogsFilters, { type ProjectTimeLogFilters } from '@/pages/project/components/ProjectTimeLogsFilters'
import { SlidersHorizontal } from 'lucide-react'

interface ProjectTimeLogsFiltersOffCanvasProps {
    open: boolean
    onOpenChange: (open: boolean) => void
    filters: ProjectTimeLogFilters
    teamMembers: { id: number; name: string }[]
    projectId: number
}

export default function ProjectTimeLogsFiltersOffCanvas({ open, onOpenChange, filters, teamMembers, projectId }: ProjectTimeLogsFiltersOffCanvasProps) {
    return (
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent side="right" className="overflow-y-auto bg-white pr-6 pb-8 pl-6 sm:max-w-md md:max-w-lg dark:bg-neutral-900">
                <SheetHeader>
                    <SheetTitle className="flex items-center gap-2 text-xl text-neutral-900 dark:text-white">
                        <SlidersHorizontal className="h-5 w-5 text-neutral-700 dark:text-neutral-300" /> Filters
                    </SheetTitle>
                    <SheetDescription className="text-sm text-neutral-500 dark:text-neutral-400">
                        Narrow down time logs by date range, member, payment status or approval status.
                    </SheetDescription>
                </SheetHeader>

                <div className="rounded-md border border-neutral-100 bg-white dark:border-neutral-800 dark:bg-neutral-800/30">
                    <ProjectTimeLogsFilters filters={filters} teamMembers={teamMembers} projectId={projectId} />
                </div>
            </SheetContent>
        </Sheet>
    )
}
