...
import type { Task as TaskType, Project as ProjectType } from '@/pages/task/types'
...
export default function QuickTrackModal({ open, onOpenChange }: QuickTrackModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-800 sm:max-w-3xl md:max-w-4xl">
        <DialogHeader>
                  ...
                  {filteredTasks.map((t) => (
                    <li key={t.id} className="flex items-center justify-between gap-3 px-3 py-2">
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-neutral-500 dark:text-neutral-400">{selectedProject?.name || t.project?.name}</div>
                      </div>
                      <Button
                        size="sm"
                        className="gap-2"
