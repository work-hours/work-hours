...
export function TimeTrackerProvider({ children }: { children: ReactNode }) {
  const [state, setState] = useState<TimeTrackerState>(() => (typeof window !== 'undefined' ? loadState() : { running: false, paused: false, task: null, startEpochMs: null, elapsedMs: 0 }))
  ...
  const stop = useCallback(async () => {
    // finalize and create time log entry
    const s = state
